...
$appResponse = $bddApp->query("SELECT value FROM misc WHERE appName='botsurcote'");
...
if($wordid < 46209){
	...
	$tweet = $word . " du cul";
	$appResponse = $bddApp->query("UPDATE misc SET value = value + 1 WHERE appName = 'botsurcote'");
	$appResponse->closeCursor();
	if(safeTweet($tweet)){
		// Post the tweet
		$postfields = array(
